...
- Pages are Server Components by default. Reach for `"use client"` only when a
  hook demands it — `app/components/Nav.tsx` is the only one today, and only
  because of `usePathname()`.
...
- Sub-page `<h1>` is `text-3xl font-semibold tracking-tight text-foreground
  sm:text-4xl`; the home hero is one step larger
...
- Shared, non-component modules do not have an established home yet. The first
  one should go under `app/lib/<domain>/` and be imported through the `@/`
  alias.
...
- Interactive targets carry `min-h-11` so touch targets stay large enough; see
  the nav links and `buttonStyles.ts` (`h-12`).
...
- One file per component or page in `__tests__/`, named `<Thing>.test.tsx`.
  The vitest `include` glob only collects `.test.tsx`.
...
- Mock `next/navigation` with `vi.mock` when the subject renders `Nav` or is
  `Nav`.
- Exact-count assertions are deliberate. `__tests__/Nav.test.tsx:38-60` asserts
  the full nav link list and a total link count so an accidental addition fails
  loudly rather than sliding through. Adding a route means updating that test
  in the same commit.
- There is no `user-event` package. Drive interactions with `fireEvent` from
  `@testing-library/react`, and await async UI with `findBy*`.
...
Run all three; they are fast (build ≈ 5s, suite ≈ 1s, lint ≈ 2s):
